...
async function importCSV() {
	try {
		...
		if (SKIP_AVAILABILITY) {
			console.log('⚠️  Availability creation SKIPPED (set SKIP_AVAILABILITY=false to enable)');
		}
		console.log('');
		...
		async function createAvailabilityEntriesBulk(productIds: mongoose.Types.ObjectId[]) {
			const priceRanges = [
				'$3.99-$4.99',
				'$4.99-$6.99',
				'$5.99-$7.99',
			...
			if (operations.length > 0) {
				...
				for (let i = 0; i < operations.length; i += BULK_BATCH_SIZE) {
					...
					if (i % 50000 === 0 || i + BULK_BATCH_SIZE >= operations.length) {
						process.stdout.write(`\r  Creating availability: ${Math.min(i + BULK_BATCH_SIZE, operations.length).toLocaleString()} / ${operations.length.toLocaleString()}`);
					}
				}
			}
		}
			...
			try {
				...
				if (batch.length >= BATCH_SIZE) {
					...
					if (!SKIP_AVAILABILITY) {
						...
						productIds.push(...products.map(p => p._id as mongoose.Types.ObjectId));
					}
		...
		parser.on('end', async () => {
			...
			if (!SKIP_AVAILABILITY) {
				console.log(`Availability entries: ${availabilityCount.toLocaleString()}`);
			}
			console.log('');
