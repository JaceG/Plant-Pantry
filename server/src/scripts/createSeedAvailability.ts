...
async function createSeedAvailability() {
	try {
		...
		if (productCount === 0) {
			...
			console.log('   Please import products first using MongoDB Compass or npm run import:products-only');
			process.exit(1);
		}
		...
		if (existingAvailability > 0) {
			console.log(`⚠️  Found ${existingAvailability.toLocaleString()} existing availability entries`);
			console.log('   Skipping creation. Delete existing entries first if you want to recreate.');
			process.exit(0);
		}
