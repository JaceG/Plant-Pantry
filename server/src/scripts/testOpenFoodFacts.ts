...
const USER_AGENT = 'PlantPantry/1.0 (plantpantry@example.com)';
...
async function exploreVeganProducts() {
  ...
  try {
    ...
    console.log(`Total vegan products found: ${result.count.toLocaleString()}`);
    ...
    result.products.slice(0, 5).forEach((p, i) => {
      ...
      console.log(`     Categories: ${p.categories_tags?.slice(0, 3).join(', ') || 'None'}`);
  ...
  try {
    ...
    console.log(`Total vegan plant milks found: ${result.count.toLocaleString()}`);
    console.log('');
  ...
  try {
    ...
    console.log(`Total vegan cheese products found: ${result.count.toLocaleString()}`);
    console.log('');
    ...
    result.products.slice(0, 5).forEach((p, i) => {
      console.log(`  ${i + 1}. ${p.product_name || 'No name'}`);
      console.log(`     Brand: ${p.brands || 'Unknown'}`);
      console.log(`     Ingredients analysis: ${p.ingredients_analysis_tags?.join(', ') || 'None'}`);
      console.log('');
    });
  } catch (error) {
    console.error('Error:', error);
