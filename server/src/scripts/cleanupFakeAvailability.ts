...
async function cleanupFakeAvailability(dryRun: boolean = true) {
  try {
    ...
    console.log(`Found ${entriesToDelete.length} cached availability entries to delete`);
    ...
    if (dryRun) {
      ...
      console.log(`✅ Deleted ${result.deletedCount} cached availability entries`);
...
if (dryRun) {
  ...
  console.log('⚠️  Running in DELETE mode - fake entries will be permanently removed\n');
}
