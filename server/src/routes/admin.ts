...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const updatedStore = await storeService.updateStore(id, filteredUpdates);
...
router.put(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id } = req.params;
      const { name, slug, logoUrl, websiteUrl, type, isActive } = req.body;
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const success = await adminService.rejectTrustedProduct(id, adminId);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (error.code === 11000) {
        next(new HttpError("A city page with this slug already exists", 409));
      } else {
        next(error);
      }
    }
    ...
    try {
      ...
      if (action === "approve") {
        for (const edit of edits) {
          ...
          await CityLandingPage.findByIdAndUpdate(edit.cityPageId, updateData);
        }
      }
    ...
    try {
      ...
      const [apiProducts, userProducts] = await Promise.all([
        Product.find({
          ...
          .select("name brand sizeOrVariant imageUrl categories archived")
          .lean(),
        UserProduct.find({
          _id: { $in: productIds },
        })
        ...
        .map((avail) => {
          ...
          return {
            ...
            reportedBy: avail.reportedBy
              ? {
                  id: (avail.reportedBy as any)._id?.toString(),
                  email: (avail.reportedBy as any).email,
                  displayName: (avail.reportedBy as any).displayName,
                }
              : null,
            // Moderator info
            moderatedBy: avail.moderatedBy
              ? {
                  id: (avail.moderatedBy as any)._id?.toString(),
                  email: (avail.moderatedBy as any).email,
                  displayName: (avail.moderatedBy as any).displayName,
                }
              : null,
      ...
      const statusCounts = {
        ...
        pending: products.filter((p: any) => p.moderationStatus === "pending")
          .length,
        rejected: products.filter((p: any) => p.moderationStatus === "rejected")
          .length,
      };
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!["confirmed", "rejected"].includes(status)) {
        throw new HttpError('Status must be "confirmed" or "rejected"', 400);
      }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { slug } = req.params;
      const { storeId, name, type, address, zipCode, websiteUrl, phoneNumber } =
        req.body;
      ...
      res.status(201).json({
        message: storeId ? "Store added to city" : "Store created in city",
        store: {
          id: store._id.toString(),
          name: store.name,
          type: store.type,
        ...
        .map((prodId) => {
          ...
          return {
            ...
            availabilities: prodAvails.map((a) => ({
              ...
              storeName: storeMap.get(a.storeId.toString())?.name || "Unknown",
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (existing) {
        throw new HttpError("Product is already available at this store", 409);
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!Array.isArray(productIds) || productIds.length === 0) {
        throw new HttpError("productIds must be a non-empty array", 400);
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      pendingReports.forEach((report) => {
        ...
        const cityKey = `${store.city || "Unknown"}, ${store.state || "??"}`;
        ...
        reportsByCity[cityKey].push({
          ...
          reportedBy: report.reportedBy
            ? {
                ...
                displayName: (report.reportedBy as any).displayName,
              }
            : null,
      ...
      const cities = Object.entries(reportsByCity).map(([city, reports]) => ({
        city,
        reports,
    ...
    try {
      ...
      const items = edits.map((edit) => ({
        ...
        storeId: edit.storeId
          ...
          : undefined,
        storeName: edit.storeId ? (edit.storeId as any).name : undefined,
        ...
        chainId: edit.chainId
          ...
          : undefined,
        chainName: edit.chainId ? (edit.chainId as any).name : undefined,
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (edit.autoApplied && edit.trustedContribution) {
        ...
        if (edit.retailerType === "chain" && edit.chainId) {
          await StoreChain.findByIdAndUpdate(edit.chainId, revertData);
        } else if (edit.retailerType === "store" && edit.storeId) {
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const [productBrandCounts, userProductBrandCounts] = await Promise.all([
        Product.aggregate([
      ...
      const uniqueBrandNames = Array.from(brandCountMap.values()).map((v) =>
        v.originalName.trim(),
      );
      ...
      for (const [parentId, childNames] of parentChildBrandsMap.entries()) {
        ...
        for (const childName of childNames) {
          totalChildProducts += brandCountMap.get(childName)?.count || 0;
        }
      ...
      for (const brandName of uniqueBrandNames) {
        ...
        const productCount = brandCountMap.get(normalizedName)?.count || 0;

        if (existingPage) {
          // Brand has a BrandPage entry
          const brandId = existingPage._id.toString();
          const childProductsTotal = childProductCountsMap.get(brandId) || 0;
          const item = {
            id: brandId,
            ...
            parentBrand: existingPage.parentBrandId
              ? {
                  id: (existingPage.parentBrandId as any)._id?.toString(),
                  brandName: (existingPage.parentBrandId as any).brandName,
                  slug: (existingPage.parentBrandId as any).slug,
                  displayName: (existingPage.parentBrandId as any).displayName,
                }
              : null,
        ...
        if (
          ...
          officialItems.push({
            ...
            parentBrand: brandPage.parentBrandId
              ? {
                  id: (brandPage.parentBrandId as any)._id?.toString(),
                  brandName: (brandPage.parentBrandId as any).brandName,
                  slug: (brandPage.parentBrandId as any).slug,
                  displayName: (brandPage.parentBrandId as any).displayName,
                }
              : null,
            childCount: childCountMap.get(brandId) || 0,
          });
      ...
      officialItems.sort((a, b) => a.displayName.localeCompare(b.displayName));
      ...
      res.json({
        brand: {
          ...
          parentBrand: brand.parentBrandId
            ? {
                id: (brand.parentBrandId as any)._id?.toString(),
                brandName: (brand.parentBrandId as any).brandName,
                slug: (brand.parentBrandId as any).slug,
                displayName: (brand.parentBrandId as any).displayName,
              }
            : null,
          childBrands,
        },
    ...
    try {
      ...
      if (parentBrandId) {
        ...
        if (!parentBrand.isOfficial) {
          throw new HttpError("Parent brand must be marked as official", 400);
        }
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      // If it was auto-applied, we need to revert the change
      if (edit.autoApplied && edit.trustedContribution && edit.brandPageId) {
        const revertData: Record<string, string> = {};
        revertData[edit.field] = edit.originalValue;
