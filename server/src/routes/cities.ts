...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!lat || !lng) {
        throw new HttpError("lat and lng query parameters are required", 400);
      }
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (grouped === "true") {
        const groupedStores = await cityService.getCityStoresGrouped(slug);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const originalValue = cityPage[field as keyof typeof cityPage] as string;
      ...
      if (isTrusted) {
        ...
        await CityLandingPage.findByIdAndUpdate(cityPage._id, updateData);
