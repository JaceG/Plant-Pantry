...
router.get('/product/:productId', optionalAuthMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const filters = {
      ...
      sortBy: sortBy as 'newest' | 'oldest' | 'helpful' | 'rating' | undefined,
    };

    const result = await reviewService.getReviews(productId, filters, currentUserId);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
router.get('/product/:productId/user', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.userId!;
    const { productId } = req.params;
...
router.get('/product/:productId/stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { productId } = req.params;
