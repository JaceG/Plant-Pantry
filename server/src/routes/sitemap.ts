...
router.get("/sitemap.xml", async (req: Request, res: Response) => {
  try {
    ...
    // Brand pages (only active ones that are not sub-brands that redirect)
    // Sub-brands with parentBrandId redirect to their parent, so exclude them from sitemap
    const brands = await BrandPage.find({
      isActive: true,
      parentBrandId: { $exists: false }, // Exclude sub-brands that redirect
    })
      .select("slug updatedAt")
      .lean();
