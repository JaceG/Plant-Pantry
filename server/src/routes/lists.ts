...
router.post('/:id/items', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!item) {
      throw new HttpError('Could not add item. List or product not found.', 404);
    }
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const success = await listService.removeItemFromList(req.userId!, listId, itemId);

      if (!success) {
        throw new HttpError('Item not found or could not be removed', 404);
      }
