...
export const adminService = {
  ...
  async approveProduct(productId: string): Promise<boolean> {
    ...
    if (pendingProduct.sourceProductId) {
      ...
      const sourceUserProduct = await UserProduct.findById(sourceId).lean();
      ...
      if (sourceUserProduct) {
        ...
        await UserProduct.findByIdAndUpdate(sourceId, {
          $set: {
            ...
            ingredientSummary: pendingProduct.ingredientSummary ?? "",
            chainAvailabilities: pendingProduct.chainAvailabilities || [],
            updatedAt: new Date(),
          },
        });
   ...
   */
  async markStoreReviewed(storeId: string, adminId: string): Promise<boolean> {
    const result = await Store.findByIdAndUpdate(
      storeId,
      {
        needsReview: false,
   ...
   */
  async rejectTrustedStore(storeId: string, adminId: string): Promise<boolean> {
    const result = await Store.findByIdAndUpdate(
      storeId,
      {
        moderationStatus: "rejected",
  ...
  async createFilter(
    ...
    const [existsInProducts, existsInUserProducts, existsInCustom] =
      await Promise.all([
        Product.findOne({
          [field]: { $regex: new RegExp(`^${normalizedValue}$`, "i") },
        }),
        UserProduct.findOne({
          [field]: { $regex: new RegExp(`^${normalizedValue}$`, "i") },
        }),
