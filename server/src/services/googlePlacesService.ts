...
// Lazy getter for API key to ensure dotenv has loaded
let _apiKeyLogged = false;
function getApiKey(): string | undefined {
	const key = process.env.GOOGLE_API_KEY;

	// Log status once on first access
	if (!_apiKeyLogged) {
		_apiKeyLogged = true;
		if (!key) {
			console.warn(
				'⚠️  GOOGLE_API_KEY is not set in environment variables'
			);
			console.warn(
				'   Make sure .env file exists and contains GOOGLE_API_KEY=your_key'
			);
		} else {
...
export const googlePlacesService = {
	...
	async autocompletePlaces(
		...
		if (!apiKey) {
			console.warn(
				'GOOGLE_API_KEY not configured, skipping Google Places API call'
			);
			return [];
		}
		...
		try {
			...
			console.log(
				'Calling Google Places Autocomplete:',
			...
			if (!response.ok) {
				...
				console.error(
					`Google Places API HTTP error: ${response.status}`,
					errorText
				);
			...
			if (data.status !== 'OK') {
				...
				if (data.status === 'REQUEST_DENIED') {
					console.error(
						'API key may be invalid or missing required permissions'
					);
				}
				return [];
			}
