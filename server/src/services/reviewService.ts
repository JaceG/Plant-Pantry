...
import { Review, Product, UserProduct } from '../models';
...
export const reviewService = {
  ...
  async approveReview(reviewId: string, adminId: string): Promise<ReviewDetail> {
    const review = await Review.findById(reviewId);
  ...
  async rejectReview(reviewId: string, adminId: string): Promise<ReviewDetail> {
    const review = await Review.findById(reviewId);
  ...
  formatReview(review: any, currentUserId?: string): ReviewDetail {
    const userIdStr = review.userId?._id?.toString() || review.userId?.toString();
    const productIdStr = review.productId?._id?.toString() || review.productId?.toString();
    const isHelpful =
      currentUserId &&
    ...
    const result: any = {
      ...
      userId: userIdStr,
      userName:
        (review.userId as any)?.displayName || 'Unknown User',
