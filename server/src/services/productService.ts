...
export const productService = {
    ...
    try {
      ...
      if (brand) {
        ...
        if (brandPage && brandPage.isOfficial) {
          ...
          if (
            !brandNames.some((n) => n.toLowerCase() === brand.toLowerCase())
          ) {
      ...
      if (locationProductIds !== null) {
        ...
        if (userProductQuery._id) {
          ...
          const existingIds = new Set(
            (userProductQuery._id.$in as mongoose.Types.ObjectId[]).map((id) =>
              id.toString(),
            ),
          );
          ...
          userProductQuery._id = {
            $in: intersectionIds.map((id) => new mongoose.Types.ObjectId(id)),
          };
          apiProductQuery._id = {
            $in: intersectionIds.map((id) => new mongoose.Types.ObjectId(id)),
          };
  ...
  async getProductById(
    ...
    const editedProduct = await UserProduct.findOne(editedProductQuery).lean();
    ...
    if (editedProduct) {
      ...
      if (!product) {
        ...
        const userProduct = await UserProduct.findOne(userProductQuery).lean();
        if (userProduct) {
          product = userProduct;
          isUserProduct = true;
        } else {
    ...
    const storeAvailabilities =
      await availabilityService.getProductAvailability(product._id.toString(), {
    ...
    if (storeAvailabilities.length > 0) {
      ...
      if (storeIdsForStock.length > 0) {
        const stockData = await Availability.find({
          ...
          storeId: {
            $in: storeIdsForStock.map((id) => new mongoose.Types.ObjectId(id)),
          },
        })
          .select(
    ...
    if (isUserProduct && (product as any).chainAvailabilities?.length > 0) {
      ...
      if (chainIds.length > 0) {
        const { StoreChain, Store: StoreModel } = await import("../models");
        ...
        enrichedChainAvailabilities = await Promise.all(
          rawChainAvails.map(async (c: any) => {
            const chainId = c.chainId?.toString?.() || String(c.chainId);
            const chainData = chainDataMap.get(chainId);
            const includeRelatedCompany = c.includeRelatedCompany !== false;
    ...
    return {
      ...
      averageRating:
        ratingStats.totalCount > 0 ? ratingStats.averageRating : undefined,
      reviewCount:
    ...
    try {
      ...
      for (const up of allUserProducts) {
        if ((up as any).sourceProductId) {
          ...
          userProductOverrides.set((up as any).sourceProductId.toString(), up);
        }
      ...
      const processedApiProducts = apiProducts.map((product) => {
        const override = userProductOverrides.get(product._id.toString());
        if (override) {
          ...
          return {
            ...
            sizeOrVariant: override.sizeOrVariant ?? product.sizeOrVariant,
  ...
  async getDiscoverProducts(limit: number = 6): Promise<ProductSummary[]> {
    try {
      ...
      console.error("Error in productService.getDiscoverProducts:", error);
      return [];
    }
  },
