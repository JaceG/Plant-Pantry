...
export const storeService = {
  ...
  async getStoresGroupedByChain(includeEmptyChains = false): Promise<{
    ...
    if (!includeEmptyChains) {
      chainGroups = chainGroups.filter((group) => group.locationCount > 0);
    }
  ...
  async checkForDuplicates(
    ...
    if (
      (input.type === "online_retailer" || input.type === "brand_direct") &&
      input.websiteUrl
    ) {
    ...
    if (similarByName.length > 0) {
      result.hasDuplicates = true;
      result.similarStores = similarByName.map((s) => toStoreSummary(s, null));
    }
