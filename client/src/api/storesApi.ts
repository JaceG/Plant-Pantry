...
import { Store, StoreListResponse, CreateStoreInput, GooglePlacePrediction, GooglePlaceDetails } from '../types/store';
...
export const storesApi = {
  ...
  createStore(input: CreateStoreInput): Promise<{ store: Store }> {
    return httpClient.post<{ store: Store }>('/stores', input);
  },

  searchPlaces(input: string, options?: {
    types?: string[];
    ...
    return httpClient.get<{ predictions: GooglePlacePrediction[] }>(`/stores/places/autocomplete?${params.toString()}`);
  },

  getPlaceDetails(placeId: string): Promise<{ place: GooglePlaceDetails }> {
    return httpClient.get<{ place: GooglePlaceDetails }>(`/stores/places/details/${placeId}`);
  },
};
