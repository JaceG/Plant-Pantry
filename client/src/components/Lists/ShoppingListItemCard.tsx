...
export function ShoppingListItemCard({ item, onRemove, isRemoving }: ShoppingListItemCardProps) {
  const { productSummary, quantity, note, availabilityHints } = item;

  return (
    <div className={`list-item-card ${isRemoving ? 'removing' : ''}`}>
          ...
          <div className="list-item-availability">
            ...
            <div className="availability-stores">
              ...
              {availabilityHints.length > 3 && (
                <span className="availability-more">+{availabilityHints.length - 3} more</span>
              )}
            </div>
          </div>
        )}
