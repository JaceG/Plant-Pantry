    ...
    <div className="chain-availability-card">
      <div className="chain-card-header" onClick={handleExpand}>
            ...
            <div className="chain-card-meta">
              {chainAvailability.chainType && (
                <span className="chain-type-badge">
                  {chainTypeLabels[chainAvailability.chainType] ||
                    chainAvailability.chainType}
                </span>
              )}
              <span className="chain-location-count">
                {chainAvailability.locationCount > 0
                  ? `${chainAvailability.locationCount} location${
                      chainAvailability.locationCount !== 1 ? "s" : ""
                    }`
                  : "All locations"}
              </span>
            </div>
        ...
        <div className="chain-card-actions">
          ...
          <span className="chain-expand-icon">{isExpanded ? "▼" : "▶"}</span>
        </div>
      </div>
        ...
        <div className="chain-card-expanded">
                  ...
                  return (
                    <div key={store.id} className="chain-store-item">
                      <div className="store-item-info">
                        <span className="store-item-name">{displayName}</span>
                        {store.address && (
          ...
          {!hasSearched && !loading && (
            <div className="chain-search-prompt">
              <p>
                Enter a location above to find {chainAvailability.chainName}{" "}
                stores near you.
              </p>
            </div>
          )}
      ...
      {chainAvailability.priceRange && (
        <div className="chain-price-info">
          ...
          <span className="price-value">{chainAvailability.priceRange}</span>
        </div>
      )}
    </div>
  );
