...
function organizeCategories(categories: string[]): Record<string, Array<{ original: string; display: string }>> {
  const organized: Record<string, Array<{ original: string; display: string }>> = {
    'Food & Snacks': [],
    'Beverages': [],
    'Dairy Alternatives': [],
    'Pantry Staples': [],
  ...
  const priorityKeywords: Record<string, string[]> = {
    'Food & Snacks': ['snack', 'cereal', 'bar', 'cracker', 'chip', 'cookie'],
    'Beverages': ['beverage', 'drink', 'juice', 'tea', 'coffee', 'soda'],
    'Dairy Alternatives': ['milk', 'cheese', 'yogurt', 'cream', 'butter', 'alternative'],
    'Pantry Staples': ['grain', 'pasta', 'rice', 'flour', 'spice', 'oil', 'sauce', 'bean'],
  };
  ...
  categories.forEach((category) => {
    ...
    if (!assigned) {
      if (
        ...
        organized['Dairy Alternatives'].push({ original: category, display });
      } else if (
        ...
        organized['Pantry Staples'].push({ original: category, display });
      } else if (
        lower.includes('food') ||
        ...
        organized['Food & Snacks'].push({ original: category, display });
      } else {
    ...
    <>
      <div className="filter-options">
        {visibleItems.map((item) => (
          <button
            key={item.value}
            className={`filter-option ${selectedValue === item.value ? 'active' : ''}`}
      ...
      {hasMore && (
        <button
          ...
          onClick={() => setShowCount(prev => Math.min(prev + 10, items.length))}
        >
      ...
      <aside className={`filter-sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="filter-content">
          ...
          <div className="filter-section">
            <h3 className="filter-section-title">Dietary & Labels</h3>
            <div className="filter-options">
              <button
          ...
          <div className="filter-section">
            ...
            {Object.entries(organizedCategories).map(([sectionName, sectionCategories]) => (
              <div key={sectionName} className="filter-subsection">
                <h4 className="filter-subsection-title">{sectionName}</h4>
                <ExpandableFilterList
