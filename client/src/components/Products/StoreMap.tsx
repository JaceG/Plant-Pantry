...
export function StoreMap({ stores, selectedStoreId, height = '400px' }: StoreMapProps) {
  ...
  useEffect(() => {
    ...
    const centerStore = storesWithCoords.find((s) => s.id === selectedStoreId) || storesWithCoords[0];

    if (!mapInstanceRef.current) {
      mapInstanceRef.current = new window.google.maps.Map(mapRef.current, {
        center: { lat: centerStore.latitude!, lng: centerStore.longitude! },
        zoom: 12,
      });
    } else {
      // Update center if selected store changed
      if (selectedStoreId) {
        const selected = storesWithCoords.find((s) => s.id === selectedStoreId);
        if (selected) {
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: `
          <div style="padding: 8px;">
            ...
            ${store.websiteUrl ? `<br/><a href="${store.websiteUrl}" target="_blank">Visit Website</a>` : ''}
          </div>
        `,
      });
