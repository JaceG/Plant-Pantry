...
export function ProximityStoreMap({
  ...
  }, [storesWithCoords.length, filteredStores.length, showAll, hasCoordinates]);
        ...
        <div className="proximity-map-controls">
          <div className="proximity-map-info">
            <span className="proximity-status">
              📍 Location set to {location.city}, {location.state} (showing all{" "}
              {storesWithCoords.length} stores)
            </span>
          </div>
        </div>
    ...
    <div className="proximity-map-wrapper">
        ...
        <div className="proximity-map-info">
          {!showAll ? (
            <span className="proximity-status">
              ...
              {filteredStores.length !== 1 ? "s" : ""} within {radius} miles of{" "}
              {location.city}
      ...
      {filteredStores.length > 0 ? (
        ...
        <div className="proximity-map-empty">
          ...
          <button onClick={() => setShowAll(true)} className="show-all-link">
            Or view all stores
          </button>
        </div>
      )}
