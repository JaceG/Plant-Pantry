...
export function ChainLocationPicker({
  ...
  const storesByState = storesWithDistance.reduce(
    (acc, store) => {
  ...
  const filteredByCity = filteredStores.reduce(
    (acc, store) => {
      ...
      <div className="location-actions">
        {!userLocation ? (
          ...
          <div className="location-active">
            <span className="location-badge">📍 Showing nearest locations</span>
            <button
              type="button"
      ...
      {filteredStores.length === 0 ? (
                  ...
                  <div className="location-distance">
                    {store.distance !== null ? (
                      <span className="distance-badge">
                        {store.distance < 1
                          ? `${(store.distance * 5280).toFixed(0)} ft`
                          : `${store.distance.toFixed(1)} mi`}
                      </span>
                    ) : (
                      <span className="distance-unknown">—</span>
                    )}
        ...
        <div className="locations-list">
          {cities.map((city) => {
            ...
            const cityStores = [...filteredByCity[city]].sort((a, b) => {
