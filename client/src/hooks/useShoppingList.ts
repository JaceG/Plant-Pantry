...
export function useShoppingList(): UseShoppingListReturn {
  ...
  const getOrCreateDefaultList =
    useCallback(async (): Promise<ShoppingListSummary | null> => {
      try {
        const response = await listsApi.getDefaultList();
        return response.list;
      } catch (err) {
        const message =
          err instanceof Error ? err.message : "Failed to get default list";
  ...
  const createList = useCallback(
    async (name: string): Promise<ShoppingListSummary | null> => {
      setState((prev) => ({ ...prev, loading: true, error: null }));
  ...
  const addItem = useCallback(
    async (listId: string, input: AddItemInput): Promise<boolean> => {
      ...
      try {
        ...
        setState((prev) => ({
          ...
          list: prev.list
            ? {
                ...prev.list,
                items: prev.list.items.filter((item) => item.itemId !== itemId),
              }
            : null,
          removingItemId: null,
        }));
  ...
  }, [authLoading, isAuthenticated, fetchLists]);
