...
export function BrandScreen() {
  ...
  const [editingField, setEditingField] = useState<BrandEditField | null>(null);
  ...
  useEffect(() => {
    if (brandPageData?.parentBrand?.slug) {
      ...
      navigate(
        `/brands/${encodeURIComponent(brandPageData.parentBrand.slug)}`,
        {
          replace: true,
        },
      );
  ...
  const handleStartEdit = useCallback(
    (field: BrandEditField) => {
      ...
      if (brandPageData) {
        currentValue =
          (brandPageData[field as keyof typeof brandPageData] as string) || "";
      }
    ...
    return (
      <div className="brand-screen">
        ...
        <SEO
          ...
          description="This brand page could not be found. No brand name was provided."
          noindex={true}
        />
  ...
  const seoDescription = brandPageData?.description
    ? brandPageData.description.slice(0, 155)
    : `Discover plant-based products from ${brandPageData?.displayName || decodedBrandName}. Browse their full product lineup and find where to buy near you.`;

  // Determine if this page should be noindexed:
  // 1. Sub-brands that redirect to parent should be noindexed
  // 2. Brands with no products should be noindexed
  const isSubBrandRedirect = !!brandPageData?.parentBrand;
  const hasNoProducts = !loadingProducts && totalCount === 0;
  const shouldNoindex = isSubBrandRedirect || hasNoProducts;

  // Build structured data for organization/brand
  const brandStructuredData = {
    "@context": "https://schema.org/",
    "@type": "Brand",
    name: brandPageData?.displayName || decodedBrandName,
    description: seoDescription,
    url: brandPageData?.websiteUrl || undefined,
  };

  return (
    <div className="brand-screen">
      <SEO
        title={`${brandPageData?.displayName || decodedBrandName} Products`}
        description={seoDescription}
        canonicalPath={`/brands/${encodeURIComponent(brandSlug)}`}
        noindex={shouldNoindex}
        structuredData={shouldNoindex ? undefined : brandStructuredData}
      />
      ...
      {brandPageData?.parentBrand && (
        <div className="sub-brand-banner">
          ...
          <Link
            to={`/brands/${encodeURIComponent(brandPageData.parentBrand.slug)}`}
        ...
        <div className="brand-hero-content">
          <nav className="breadcrumb">
            ...
            {brandPageData?.parentBrand ? (
              ...
              <span>{brandPageData?.displayName || decodedBrandName}</span>
            )}
            {isAuthenticated && (
              <button
          ...
          {brandPageData?.isOfficial && (
            <span className="official-brand-badge">✓ Official Brand Page</span>
          )}
          ...
          <p className="brand-subtitle">
            ...
            {!loadingStores && storesData && storesData.totalStores > 0 && (
              <>
                {" • "}
      ...
      <div className="brand-description-section">
        <div className="brand-description-content">
          <div className="brand-description-placeholder">
            ...
            <h2>About {brandPageData?.displayName || decodedBrandName}</h2>
              ...
              <div className="edit-field-container edit-description-container">
                ...
                <div className="edit-actions">
                  ...
                  <button
                    ...
                    disabled={
                      submittingEdit ||
                      editValue === (brandPageData?.description || "")
                    }
                  >
                    {submittingEdit ? "Submitting..." : "Submit"}
                  </button>
              ...
              <div className="edit-field-container">
                ...
                <div className="edit-actions">
                  ...
                  <button
                    ...
                    disabled={
                      submittingEdit ||
                      editValue === (brandPageData?.websiteUrl || "")
                    }
                  >
                    {submittingEdit ? "Submitting..." : "Submit"}
                  </button>
              ...
              brandPageData.childBrands.length > 0 && (
                <div className="also-known-as">
                  ...
                  <div className="aka-brands">
                    {brandPageData.childBrands.map((child, index) => (
                      <span key={child.id} className="aka-brand">
                        {child.displayName}
                        {index < brandPageData.childBrands!.length - 1
                          ? ", "
                          : ""}
                      </span>
                    ))}
            ...
            <p className="brand-claim-notice">
              Are you the brand owner? Contact us to claim this page and add
              your brand story, logo, and more.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="brand-tabs">
        <button
          className={`brand-tab ${activeTab === "products" ? "active" : ""}`}
        ...
        <button
          className={`brand-tab ${activeTab === "stores" ? "active" : ""}`}
          onClick={() => setActiveTab("stores")}
        >
          <span className="tab-icon">📍</span>
          Where to Buy
          <span className="tab-count">{storesData?.totalStores || 0}</span>
        </button>
      </div>
        ...
        {activeTab === "products" ? (
          ...
          <div className="stores-section">
            {loadingStores ? (
              ...
              <div className="brand-empty">
                <p>No store locations found for this brand yet.</p>
                <p className="empty-note">
                  Know where to find {decodedBrandName} products? Help us by
                  reporting availability on any product page!
                </p>
              </div>
            ) : (
              <>
                ...
                {storesData.independentStores.length > 0 && (
                  <div className="stores-group">
                    <h2 className="section-title">
                      ...
                      Independent Stores
                      <span className="section-count">
                        {storesData.independentStores.length} locations
                      </span>
                    </h2>
                    <div className="independent-stores">
                      {storesData.independentStores.map((store: BrandStore) => (
                        <div key={store.id} className="store-card">
                          <div className="store-info">
                ...
                {storesData.onlineStores.length > 0 && (
                  <div className="stores-group">
                    <h2 className="section-title">
                      ...
                      Online Retailers
                      <span className="section-count">
                        {storesData.onlineStores.length} stores
                      </span>
                    </h2>
                    <div className="online-stores">
                      {storesData.onlineStores.map((store: BrandStore) => (
                        <div key={store.id} className="store-card online">
                          <div className="store-info">
                            <span className="store-name">{store.name}</span>
                          </div>
                        </div>
