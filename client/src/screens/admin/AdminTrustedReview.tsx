...
export function AdminTrustedReview() {
	...
	const fetchProducts = useCallback(async () => {
		try {
			setLoading(true);
			const response = await adminApi.getTrustedProductsPendingReview(
				page,
				20
			);
	...
	const fetchStores = useCallback(async () => {
		try {
			setLoading(true);
			const response = await adminApi.getTrustedStoresPendingReview(
				page,
				20
			);
			...
			<div className='admin-trusted-review'>
				<header className='page-header'>
					<div>
						...
						<p className='page-subtitle'>
							Content from trusted contributors is live but needs
							review. Rejecting will remove it from public view.
						</p>
					</div>
				</header>
				...
				<div className='review-tabs'>
					...
					<button
						...
						🏪 Stores{' '}
						{storeTotal > 0 && (
							<span className='tab-count'>{storeTotal}</span>
						)}
					</button>
				</div>
					...
					<div className='review-content'>
						{products.length === 0 ? (
							<div className='empty-state'>
								...
								<h2>All Caught Up!</h2>
								<p>
									No trusted contributor products pending
									review.
								</p>
							</div>
						) : (
					...
					<div className='review-content'>
						{stores.length === 0 ? (
							<div className='empty-state'>
								...
								<h2>All Caught Up!</h2>
								<p>
									No trusted contributor stores pending
									review.
								</p>
							</div>
						) : (
