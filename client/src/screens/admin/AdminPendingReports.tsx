        ...
        {cities.length === 0 ? (
          ...
          <div className="cities-list">
            {cities.map((cityGroup) => {
              const isExpanded = expandedCities.has(cityGroup.city);
              ...
              return (
                <div
                  key={cityGroup.city}
                  className={`city-group ${isExpanded ? "expanded" : ""}`}
                >
                  <div
                  ...
                  >
                    <div className="city-info">
                      ...
                      <h2 className="city-name">{cityGroup.city}</h2>
                      <span className="city-count">
                        {cityGroup.count}{" "}
                        {cityGroup.count === 1 ? "report" : "reports"}
                      </span>
                    </div>
                    {isExpanded && (
                      <div
                      ...
                      >
                        {allSelected ? (
                          <button
                            onClick={() => deselectAllInCity(cityGroup)}
                          ...
                          <button
                            onClick={() => selectAllInCity(cityGroup)}
                            className="select-all-btn"
                          >
                    ...
                    <div className="reports-list">
                      {cityGroup.reports.map((report) => (
                        <ReportCard
                          ...
                          onToggleSelect={() => toggleReport(report.id)}
                          onApprove={() =>
                            handleModerate(report.id, "confirmed")
                          }
                          onReject={() => handleModerate(report.id, "rejected")}
  ...
  return (
    <div className={`report-card ${isSelected ? "selected" : ""}`}>
      ...
      <div className="report-product">
        <div className="product-image">
          {report.productImageUrl ? (
            <img src={report.productImageUrl} alt={report.productName} />
          ) : (
            <span>🌱</span>
          )}
        </div>
        <div className="product-details">
          <Link to={`/products/${report.productId}`} className="product-name">
            {report.productName}
          </Link>
      ...
      <div className="report-info">
        {report.priceRange && (
          <span className="info-item price">💵 {report.priceRange}</span>
        )}
        {report.notes && (
      ...
      <div className="report-meta">
        ...
        <span className="reported-at">{formatDate(report.createdAt)}</span>
      </div>

      <div className="report-actions">
        <button onClick={onApprove} className="approve-btn" title="Approve">
          ✓
        </button>
        <button onClick={onReject} className="reject-btn" title="Reject">
          ×
        </button>
      </div>
    </div>
