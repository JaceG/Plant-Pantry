...
export function AdminProducts() {
  ...
  const [userGeneratedProducts, setUserGeneratedProducts] = useState<
    AdminProduct[]
  >([]);
  ...
  const fetchUserGeneratedProducts = useCallback(async () => {
    try {
      setLoading(true);
      const response = await adminApi.getUserGeneratedProducts(
        userGeneratedPage,
        20,
      );
        ...
        <header className="page-header">
          <div>
            ...
            <p className="page-subtitle">
              {activeTab === "pending"
                ? pendingTotal > 0
                  ? `${pendingTotal} product${
                      pendingTotal !== 1 ? "s" : ""
                    } pending approval`
                  : "No products pending approval"
                : activeTab === "user-generated"
                  ? `${userGeneratedTotal} user-generated product${
                      userGeneratedTotal !== 1 ? "s" : ""
                    }`
                  : `${archivedTotal} archived product${
                      archivedTotal !== 1 ? "s" : ""
                    }`}
            </p>
          </div>
        ...
        <div className="admin-tabs">
          ...
          <button
            className={`tab-button ${
              activeTab === "user-generated" ? "active" : ""
            }`}
                  ...
                  >
                    ...
                    <div className="product-info">
                      <h3 className="product-name">
                        ...
                        {product.isEditSuggestion &&
                          product.originalProduct &&
                          product.name !== product.originalProduct.name && (
                            <span className="changed-indicator">
                              (was: {product.originalProduct.name})
                            </span>
                          )}
                      </h3>
                      <p className="product-brand">
                        ...
                        {product.isEditSuggestion &&
                          product.originalProduct &&
                          product.brand !== product.originalProduct.brand && (
                            <span className="changed-indicator">
                              (was: {product.originalProduct.brand})
                            </span>
                          )}
                      </p>

                      {product.isEditSuggestion && product.originalProduct && (
                        <div className="edit-changes-summary">
                          ...
                          <div className="changes-diff">
                            {product.name !== product.originalProduct.name && (
                              <div className="diff-row">
                                ...
                                <span className="diff-original">
                                  {product.originalProduct.name || "(empty)"}
                                </span>
                            ...
                            {product.brand !==
                              ...
                              <div className="diff-row">
                                <span className="diff-label">Brand:</span>
                                <span className="diff-original">
                                  {product.originalProduct.brand || "(empty)"}
                                </span>
                                <span className="diff-arrow">→</span>
                                <span className="diff-new">
                                  {product.brand || "(empty)"}
                            ...
                            {product.description !==
                              product.originalProduct.description && (
                              <div className="diff-row">
                            ...
                            {product.sizeOrVariant !==
                              product.originalProduct.sizeOrVariant && (
                              <div className="diff-row">
                                ...
                                <span className="diff-arrow">→</span>
                                <span className="diff-new">
                                  {product.sizeOrVariant || "(deleted)"}
                                </span>
                              </div>
                            )}
                            {JSON.stringify(product.categories) !==
                              JSON.stringify(
                                product.originalProduct.categories,
                              ) && (
                              <div className="diff-row">
                                <span className="diff-label">Categories:</span>
                                <span className="diff-original">
                                  {product.originalProduct.categories?.length
                            ...
                            {JSON.stringify(product.tags) !==
                              JSON.stringify(product.originalProduct.tags) && (
                              <div className="diff-row">
                                <span className="diff-label">Tags:</span>
                                <span className="diff-original">
                                  {product.originalProduct.tags?.length
                            ...
                            {product.imageUrl !==
                              product.originalProduct.imageUrl && (
                              <div className="diff-row diff-row-image">
                                ...
                                <div className="diff-images">
                                  <div className="diff-image-box">
                                    ...
                                    {product.originalProduct.imageUrl ? (
                                      <img
                                        src={product.originalProduct.imageUrl}
                                        alt="Original"
              ...
              : activeTab === "user-generated"
                ...
                : // Archived products view
                  archivedProducts.map((product) => (
                    <div key={product.id} className="product-card archived">
