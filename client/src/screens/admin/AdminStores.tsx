...
export function AdminStores() {
  ...
  const fetchPendingStores = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load pending stores",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleEditFormChange = (field: keyof EditStoreData, value: string) => {
  ...
  const handleChainFormChange = (field: keyof ChainFormData, value: string) => {
    ...
    <AdminLayout>
        ...
        <header className="page-header">
          <div className="page-header-main">
            ...
            <div className="header-actions">
              <Button variant="secondary" size="sm" onClick={handleCreateChain}>
                + Add Chain
              </Button>
            </div>
          </div>
                ...
                {selectedStoreIds.size > 0 && (
                  <div className="bulk-action-bar">
                    <span className="selection-count">
                      ...
                      {selectedStoreIds.size !== 1 ? "s" : ""} selected
                    </span>
                    ...
                    <tr>
                      <th className="checkbox-cell">
                        <input
                          type="checkbox"
                          checked={
                            selectedStoreIds.size === sortedStores.length &&
                      ...
                      <th
                        className="sortable-header"
                        onClick={() => handleSort("regionOrScope")}
                      >
                        Region{" "}
                        <span className="sort-indicator">
                  ...
                  <tbody>
                    {sortedStores.map((store) => (
                      <tr
                        key={store.id}
                        className={
                          selectedStoreIds.has(store.id) ? "selected" : ""
                        }
                      >
                        <td className="checkbox-cell">
                          <input
                            ...
                            onChange={() =>
                              handleToggleStoreSelection(store.id)
                            }
                          />
                        </td>
        ...
        {viewMode === "grouped" && groupedData && (
          <div className="grouped-stores-view">
            ...
            <div className="chains-section">
              ...
              {groupedData.chains.length === 0 ? (
                ...
                <div className="chains-list">
                  {groupedData.chains.map(({ chain, stores: chainStores }) => (
                      ...
                      {expandedChains.has(chain.id) && (
                        <div className="chain-stores">
                          {chainStores.map((store) => (
                            <div key={store.id} className="chain-store-item">
                              <span className="store-icon">
                                {getStoreTypeIcon(store.type)}
                              </span>
            ...
            <div className="independent-section">
              <h2 className="section-title">
                🏪 Independent Stores ({groupedData.independentStores.length})
              </h2>
              ...
              {groupedData.independentStores.length === 0 ? (
                ...
                <div className="independent-stores-list">
                  {groupedData.independentStores.map((store) => (
                    <div key={store.id} className="independent-store-item">
                      <span className="store-icon">
                        {getStoreTypeIcon(store.type)}
                      </span>
                      <div className="store-details">
              ...
              <>
                    ...
                    <div key={store.id} className="pending-store-card">
                      ...
                      <div className="pending-store-actions">
                        <Button
                          ...
                          isLoading={moderatingStoreId === store.id}
                          disabled={moderatingStoreId !== null}
                        >
                          ✅ Approve
                        </Button>
                        <Button
                ...
                {pendingTotal > 20 && (
                  <div className="pagination">
                    ...
                    <span className="page-info">
                      Page {pendingPage} of {Math.ceil(pendingTotal / 20)}
                    </span>
                    <Button
          ...
          >
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleEditFormChange("chainId", e.target.value)
                  }
                >
                  <option value="">— Independent Store —</option>
                  {chains.map((chain) => (
                    <option key={chain.id} value={chain.id}>
                      {chain.name} ({chain.locationCount} locations)
                    </option>
                  ))}
                </select>
              </div>
              ...
              {editForm.chainId && (
                <div className="form-group">
                  <label htmlFor="edit-location-id">Location Identifier</label>
                  <input
                    type="text"
                    id="edit-location-id"
                    value={editForm.locationIdentifier}
                    onChange={(e) =>
                      handleEditFormChange("locationIdentifier", e.target.value)
                    }
              ...
              <div className="form-group">
                ...
                >
                  <option value="brick_and_mortar">🏪 Physical Store</option>
                  <option value="online_retailer">🌐 Online Retailer</option>
                  <option value="brand_direct">🏷️ Brand Direct</option>
                </select>
              </div>

              <div className="form-group">
                <label htmlFor="edit-region">Region/Scope *</label>
                <input
                  type="text"
                  id="edit-region"
                  value={editForm.regionOrScope}
                  onChange={(e) =>
                    handleEditFormChange("regionOrScope", e.target.value)
                  }
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleEditFormChange("websiteUrl", e.target.value)
                  }
                  placeholder="https://..."
                />
              </div>
            ...
            <div className="modal-footer">
              ...
              <Button
                ...
                disabled={
                  !editForm.name.trim() || !editForm.regionOrScope.trim()
                }
              >
                Save Changes
              </Button>
      ...
      {showChainModal && (
        <div className="modal-overlay" onClick={() => setShowChainModal(false)}>
          ...
          >
            <div className="modal-header">
              <h2>{editingChain ? "Edit Chain" : "Create Chain"}</h2>
              <button
            ...
            <div className="modal-body">
              ...
              {editingChain && (
                <div className="chain-meta">
                  <p>
                    <strong>Locations:</strong> {editingChain.locationCount}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="modal-footer">
              {editingChain && (
                <Button
                  variant="secondary"
                  onClick={() => handleDeleteChain(editingChain)}
                  disabled={chainSaveLoading || editingChain.locationCount > 0}
                >
                  🗑️ Delete
                </Button>
              )}
          ...
          <div className="modal bulk-assign-modal">
            ...
            <div className="modal-body">
              <p className="bulk-assign-info">
                ...
                {selectedStoreIds.size !== 1 ? "s" : ""} to a chain:
              </p>

              <div className="form-group">
                <label htmlFor="bulk-chain-select">Select Chain</label>
                <select
                  id="bulk-chain-select"
                  value={bulkAssignChainId}
                  onChange={(e) => setBulkAssignChainId(e.target.value)}
                ...
                <ul>
                  ...
                  {selectedStoreIds.size > 5 && (
                    <li className="more-stores">
                      ...and {selectedStoreIds.size - 5} more
                    </li>
                  )}
                </ul>
              </div>
