...
export function AdminCityContentEdits() {
  ...
  const handleApprove = async (editId: string) => {
    setProcessing(true);
    try {
      await adminApi.approveCityContentEdit(editId, reviewNote || undefined);
  ...
  const handleReject = async (editId: string) => {
    setProcessing(true);
    try {
      await adminApi.rejectCityContentEdit(editId, reviewNote || undefined);
  ...
  const handleBulkReview = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      const res = await adminApi.bulkReviewCityContentEdits(editIds, action);
              ...
              >
                ...
                {edit.reason && (
                  <div className="edit-reason">
                    ...
                    <span className="reason-text">{edit.reason}</span>
                  </div>
                )}
                ...
                <div className="edit-meta">
                  ...
                  {edit.status !== "pending" && (
                    <span className={`status-badge ${edit.status}`}>
                      {edit.status}
                    </span>
                  )}
                </div>
                  ...
                  <div className="edit-actions">
                    {reviewingEdit === edit.id ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={() => setReviewingEdit(edit.id)}
                ...
                {edit.reviewNote && (
                  <div className="review-note">
                    <span className="note-label">Review note:</span>
                    <span className="note-text">{edit.reviewNote}</span>
                  </div>
                )}
              </div>
            ))}
