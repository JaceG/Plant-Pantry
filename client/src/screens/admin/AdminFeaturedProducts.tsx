...
export function AdminFeaturedProducts() {
	...
	const handleSearch = async () => {
		...
		try {
			...
			setSearchResults(
				response.items.filter((p) => !featuredIds.has(p.id))
			);
		} catch (err) {
			...
			<div className='admin-featured-products'>
				<header className='page-header'>
					...
					<p className='page-subtitle'>
						Manage which products appear on the homepage featured
						section
					</p>
				</header>
				...
				<section className='featured-list-section'>
					...
					{featuredProducts.length === 0 ? (
						<div className='empty-state'>
							<p>
								No featured products yet. Use the search above
								to add some!
							</p>
						</div>
					) : (
