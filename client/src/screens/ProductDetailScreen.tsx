...
import { Button, Toast, SEO } from "../components";
...
export function ProductDetailScreen() {
  ...
  useEffect(() => {
    ...
    const hasChainAvailability =
      product.chainAvailabilities && product.chainAvailabilities.length > 0;
    ...
    const loadStores = async () => {
      try {
        ...
        if (hasChainAvailability) {
          const chainStorePromises = (product.chainAvailabilities || []).map(
            async (chainAvail) => {
              try {
                const response = await storesApi.getChainLocations(
                  chainAvail.chainId,
                  {
                    includeRelated: chainAvail.includeRelatedCompany,
                  },
                );
  ...
  const handleConfirmAvailability = useCallback(
    async (storeId: string) => {
      ...
      try {
        const response = await productsApi.confirmAvailability(id, storeId);
  ...
  const openConfirmModal = useCallback((storeId: string, storeName: string) => {
    ...
    return (
      <div className="product-detail-screen">
        ...
        <SEO
          ...
          description="This product could not be found. It may have been removed or the URL is incorrect."
          noindex={true}
        />
        <div className="detail-container">
  ...
  // Build product description for SEO
  const seoDescription = product.description
    ? `${product.description.slice(0, 150)}${product.description.length > 150 ? "..." : ""}`
    : `Find ${product.name} by ${product.brand} at vegan-friendly stores. ${product.tags?.length ? `Tags: ${product.tags.slice(0, 3).join(", ")}` : ""}`;
    ...
    <div className="product-detail-screen">
      ...
      <div className="detail-container">
              ...
              <div className="archived-badge-product">
                ...
                {product._archivedAt && (
                  <span className="archived-date">
                    (Archived{" "}
                    {new Date(product._archivedAt).toLocaleDateString()})
                  </span>
                )}
              </div>
            )}
        ...
        <section className="availability-section">
          ...
          {groupedAvailability.onlineStores.length > 0 && (
            <div className="online-stores-section">
              ...
              <div className="availability-grid">
                {groupedAvailability.onlineStores.map((avail) => (
                  <div
                    key={avail.storeId}
                  ...
                  >
                    <div className="store-header">
                      ...
                      {avail.status === "known" ? (
                        ...
                        >
                          {availabilityStatusLabels[avail.status] ||
                            avail.status}
                        </span>
                      ) : (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            openConfirmModal(avail.storeId, avail.storeName);
                          }}
                    ...
                    <div className="store-stock-status">
                      <StockStatusButton
                        ...
                        currentStatus={avail.stockStatus}
                        lastReportAt={avail.lastStockReportAt}
                        recentInStockCount={avail.recentInStockCount}
                        recentOutOfStockCount={avail.recentOutOfStockCount}
                        compact
                        onReportSuccess={() => refresh()}
                        onAuthRequired={() => setShowRegistrationModal(true)}
                      />
                    </div>
                  </div>
                ))}
          ...
          {groupedAvailability.independentPhysicalStores.length > 0 && (
            <div className="independent-stores-section">
              <h3 className="availability-subsection-title">
                🏪 Independent Stores
              </h3>
              <IndependentStoresCard
                stores={groupedAvailability.independentPhysicalStores}
            ...
            product.chainAvailabilities.length > 0 && (
              <div className="chain-availability-section">
                ...
                <div className="chain-availability-cards">
                  {product.chainAvailabilities.map((chainAvail) => (
          ...
          {groupedAvailability.chainGroups.length > 0 && (
            <div className="chain-availability-badges">
              ...
              <div className="chain-badges">
                {groupedAvailability.chainGroups.map(({ chain, items }) => (
                  <Link
                    key={chain.id}
          ...
          !product.chainAvailabilities?.length ? (
            <div className="availability-empty">
              <p>
                Availability information is not yet available for this product.
              </p>
              <p className="availability-empty-note">
                Help us grow our database by reporting where you've found this
                product!
              </p>
            </div>
          ) : null}
      ...
      {confirmModalStore && (
        ...
        >
          <div className="confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              Is <strong>{product?.name}</strong> still available at{" "}
              <strong>{confirmModalStore.storeName}</strong>?
            </p>
            <div className="confirm-modal-actions">
              ...
              <button
                ...
                onClick={() =>
                  handleConfirmAvailability(confirmModalStore.storeId)
                }
                disabled={confirmingStoreId === confirmModalStore.storeId}
              >
